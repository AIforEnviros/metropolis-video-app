...
Session format **v1.19** retains each accent's timestamp, optional scrub mode, range, cue position, and speed per tab and video slot in `tabs.accentPoints`, alongside multiple MIDI mappings for discrete actions. Normal cue identities used to preserve post-accent navigation are repaired automatically when an older session is loaded. Older accents without cue-position data migrate to **Centre**; v1.12 accents also migrate to **None**, a 2-second range, and 1x speed. Older sessions without accent data load with all four accent positions unset.
