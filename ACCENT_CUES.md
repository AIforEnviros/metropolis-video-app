...
Session format **v1.13** saves each accent's timestamp, optional scrub mode, range, and speed per tab and video slot in `tabs.accentPoints`. v1.12 accents migrate to **None**, a 2-second range, and 1x speed. Older sessions without accent data load with all four accent positions unset.
