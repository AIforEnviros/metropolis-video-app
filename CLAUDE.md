...
- Session format v1.14 adds portable Collect All & Save packages while retaining scrub settings and four direct-trigger accents per video slot
...
## MIDI Implementation History

**Historical branch:** `feature/midi-mapping` (its implementation has since been incorporated into the main Electron codebase)
...
**Original MIDI session format (v1.4; current application format is v1.14):**
```json
{
  "version": "1.4",
  "keyboardShortcuts": { ... },
...
- Physical MIDI latency, external-display behavior, and representative Metropolis footage still require hardware acceptance testing
- Do not reintroduce Bounce mode; it was intentionally removed from the active application
